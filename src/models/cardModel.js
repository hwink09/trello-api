...
/**
 * Xử lí cập nhật thêm hoặc xoá thành viên trong card dựa theo action
 * sẽ dùng $push để thêm thành viên và $pull để xoá thành viên
 */
const updateMembers = async (cardId, incomingMemberInfo) => {
  try {
    // Tạo ra một biến updateCondition ban đầu là rỗng
    let updateCondition = {}
    if (incomingMemberInfo.action === CARD_MEMBERS_ACTION.ADD) {
      updateCondition = { $push: { memberIds: new ObjectId(incomingMemberInfo.userId) } }
    }
    if (incomingMemberInfo.action === CARD_MEMBERS_ACTION.REMOVE) {
